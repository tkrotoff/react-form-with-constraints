...
import { FieldFeedback, FieldFeedbackProps, FieldFeedbacks, FieldFeedbacksProps } from './index';
...
function findFieldFeedbackList(wrapper: ShallowWrapper<{}, {}>, inputName: string, fieldFeedbackType: React.ComponentType<FieldFeedbacksProps>) {
  const fieldFeedbacksList = wrapper.findWhere(node => {
    let found = false;
    if (node.type() === fieldFeedbackType) {
...
export function getFieldFeedbacksMessages(
  inputs: ShallowWrapper<React.InputHTMLAttributes<HTMLInputElement>, {}>,
  fieldFeedbackType: React.ComponentType<FieldFeedbacksProps> = FieldFeedbacks
) {
  ...
  inputs.forEach(input => {
    ...
    const fieldFeedbackList = findFieldFeedbackList(input.parents(), name!, fieldFeedbackType);
